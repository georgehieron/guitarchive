<template>
  <main class="guitarchive wrapper">
    ...
    <ul class="guitar-list padded">
      <li v-for="gtr in filteredGuitars" :key="gtr.modelSlug">
        <NuxtLink :to="`${gtr.brandSlug}/${gtr.modelSlug}`">
          <span>{{ gtr.name }}</span>
          <picture>
            <!-- <img :src="`${gtr.img}`" :alt="`${gtr.name} image`" onerror="this.onerror=null;this.parentElement.classList.add('image--notfound');"/> -->
            <img
              :src="`https://www.levytukku.fi/images/products/rg550-dy-00-03-cu-body-top_orig.jpg`"
              :alt="`${gtr.name} image`"
              onerror="this.onerror=null;this.parentElement.classList.add('image--notfound');"
            />
          </picture>
        </NuxtLink>
      </li>
